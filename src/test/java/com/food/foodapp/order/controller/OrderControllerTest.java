...
class OrderControllerTest {
    ...
    void placeOrder_returns201() throws Exception {
        when(orderService.placeOrder(any())).thenReturn(order(700L, OrderStatus.CONFIRMED));
    ...
    void getOrder_returnsOrder() throws Exception {
        when(orderService.getOrder(700L)).thenReturn(order(700L, OrderStatus.CONFIRMED));
    ...
    void trackOrder_returnsTrackingInfo() throws Exception {
        when(orderService.trackOrder(700L)).thenReturn(OrderTrackingResponse.builder()
                ...
                .steps(List.of(
                        TrackingStepResponse.builder().status(OrderStatus.CONFIRMED).completed(true).current(true).build(),
        ...
        mockMvc.perform(get("/api/v1/orders/700/track"))
                ...
                .andExpect(jsonPath("$.steps[0].current").value(true));
    }
    ...
    void cancelOrder_returns409_whenNoLongerCancellable() throws Exception {
        when(orderService.cancelOrder(eq(700L)))
                .thenThrow(new InvalidOrderStatusTransitionException("Order 700 cannot move from DELIVERED to CANCELLED"));
    ...
    @Test
    void confirmDelivery_returns200_withDeliveredOrder() throws Exception {
        when(orderService.confirmDelivery(eq(700L))).thenReturn(order(700L, OrderStatus.DELIVERED));

        mockMvc.perform(put("/api/v1/orders/700/confirm-delivery"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("DELIVERED"));
    }

    @Test
    void confirmDelivery_returns409_whenNotOutForDelivery() throws Exception {
        when(orderService.confirmDelivery(eq(700L)))
                .thenThrow(new InvalidOrderStatusTransitionException("Order 700 cannot move from CONFIRMED to DELIVERED"));
    ...
    @Test
    void confirmDelivery_returns404_whenNotOwnedByCaller() throws Exception {
        when(orderService.confirmDelivery(eq(700L))).thenThrow(new OrderNotFoundException("Order not found: 700"));
